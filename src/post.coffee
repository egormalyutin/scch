...
send = (nick, chatter, message) ->
	...
	request.post({url:'http://'+chatter+':' + port, form: {message: cpass(message), nick: cpass(nick)}})
	request.post({url:'http://127.0.0.1:' + port, form: {message: cpass(message), nick: cpass('You')}})
	READ '>>> '.blue, (message) ->
		send(nick, chatter, message)
